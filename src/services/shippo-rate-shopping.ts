...
export async function shippoRateQuoteFromTo(params: {
  fromCity: string;
  fromState: string;
  fromZip?: string;
  toCity: string;
  ...
  const { fromCity, fromState, fromZip, toCity, toState, toZip, weightLbs } = params;
  const apiKey = config.shippo.apiKey;
  if (!apiKey) throw new Error('Shippo API key is not configured');
  if (config.shippo.mockMode) {
    const base = 5;
    const perLb = 0.5;
    return { amount: base + perLb * Math.max(weightLbs, 0), currency: 'USD', provider: 'mock-shippo', raw: { mock: true } };
  }
  const from = buildAddress(fromCity, fromState, fromZip);
  ...
  if (!res.ok) throw new Error(`Shippo rate request failed (${res.status}): ${await res.text()}`);
  const json = await res.json();
  const rates: ShippoRate[] = Array.isArray(json?.rates) ? json.rates : [];
  if (!rates.length) throw new Error('Shippo did not return rates');
  ...
  if (!best || !Number.isFinite(Number(best.amount))) throw new Error('Shippo returned no valid rate amounts');
  return { amount: Number(best.amount), currency: best.currency || 'USD', provider: best.provider, raw: json };
}
